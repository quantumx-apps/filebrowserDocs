...
3. **Context shortcuts** — While browsing or editing:
   ...
   - **Storage quota editor** — folder quota create, update, and remove events for the current path
...
| **quotas** | Folder quota create, update, delete |
...
| **quotaCreate**, **quotaUpdate**, **quotaDelete** | Folder storage quota create, update, and remove (admin API and Storage quota dialog) |
...
Folder quota events include source, path, and field-level details (`limitBytes`, `meter`, optional `userId` binding).
...
- {{< doclink path="features/quotas/" text="Storage quotas" />}} — folder quota changes appear in the **quotas** activity scope
